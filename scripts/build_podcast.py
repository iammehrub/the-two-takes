...
"""
    ...
    if not m:
        raise RuntimeError(
            "Gemini did not return the expected episode format"
        )

    title, topic, keywords, hook, script = [
        x.strip()
        for x in m.groups()
    ]
    ...
    if len(script.split()) < 1100:
        raise RuntimeError(
            "Generated script was too short; refusing to publish"
        )
...
def split_script_for_tts(script, max_chars=6000):
    """
    Split the podcast by complete dialogue turns.

    This prevents Gemini TTS from receiving one very large request,
    which was causing the GitHub Action to time out.
    """
    ...
    if not dialogue_lines:
        raise RuntimeError(
            "Could not find Himel/Niha dialogue in generated script"
        )
    ...
    for line in dialogue_lines:
        ...
        # If adding this dialogue would make the chunk too large,
        # start a new chunk.
...
"""
    ...
    for attempt in range(1, 5):
        ...
        try:
            ...
            print(
                f"  Gemini TTS attempt {attempt}/4..."
            )
            ...
            if r.status_code >= 400:
                ...
                raise RuntimeError(
                    f"Gemini TTS API {r.status_code}: "
                    f"{r.text[:1200]}"
                )
            ...
            last_error = (
                f"Gemini TTS timeout on attempt "
                f"{attempt}: {e}"
            )
    ...
    raise RuntimeError(
        f"Gemini TTS failed after 4 attempts. "
        f"Last error: {last_error}"
    )
...
def tts(script):
    ...
    chunks = split_script_for_tts(
        script,
        max_chars=6000
    )

    print(
        f"Splitting podcast into {len(chunks)} TTS chunks."
    )
    ...
    for i, chunk in enumerate(chunks, 1):
        ...
        # This also reduces the chance of hitting rate limits.
        if i < len(chunks):
            time.sleep(2)
...
def pexels_videos(keywords):
    ...
    for i, (url, dur, page, creator) in enumerate(out):
        ...
        print(
            f"Downloading Pexels clip {i + 1}/{len(out)}..."
        )
    ...
    if not out:
        ...
        raise RuntimeError(
            "No Pexels clips were returned. "
            "Check PEXELS_API_KEY or try again later."
        )

    print(
        f"Downloaded {len(out)} Pexels clips."
    )
...
def render_video(
    ...
    if not processed:
        ...
        raise RuntimeError(
            "No processed video clips available."
        )

    concat = WORK / "concat.txt"

    # Repeat clips until we have enough material.
    with open(
        concat,
        "w",
        encoding="utf-8"
...
def youtube_upload(
    ...
    description = (
        ...
        "Visual credits: Pexels "
        "(used via the Pexels API)."
    )
    ...
    print(
        f"Published: https://youtu.be/{vid}"
    )
...
def main():
    ...
    print(
        f"Got {len(news)} headlines"
    )

    title, topic, keywords, hook, script = make_episode(
        news
    )
    ...
    print(
        f"Script length: {len(script.split())} words"
    )
    ...
    print(
        f"Audio duration: {secs / 60:.1f} minutes"
    )

    if secs < 8 * 60 or secs > 12.5 * 60:

        print(
            "WARNING: Audio duration is outside "
            "the 8–12.5 minute target range."
        )

        print(
            "Continuing with generated result."
        )
